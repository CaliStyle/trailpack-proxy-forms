...
module.exports = {
   ...
   */
  configure: (app) => {
    // const proxyForms = app.services.proxyFormsService.proxyForms
    return Promise.resolve()
  },

  /**
   * addRoutes - Add the Proxy Forms controller routes
   * @param app
   */
  addRoutes: (app) => {
    const prefix = _.get(app.config, 'proxyForms.prefix') || _.get(app.config, 'footprints.prefix')
   ...
   */
  copyDefaults: (app) => {
    app.config.proxyFormsDefaults = _.clone(app.config.proxyForms)
    return Promise.resolve({})
  }
}
